...
Principal UX/UI Redesign — Executive Dark Glassmorphism Edition
"""
...
<style>
  ...
  .stApp {
    background: radial-gradient(circle at 50% 0%, #151b2e 0%, var(--bg-primary) 70%);
  }

  /* Sidebar styling */
  [data-testid="stSidebar"] {
    background: linear-gradient(180deg, #111625 0%, #0c0f18 100%) !important;
...
CHART_THEME = {
    "plot_bgcolor": "rgba(22, 29, 45, 0.5)",
    "paper_bgcolor": "rgba(22, 29, 45, 0.0)",
    "font_color": "#e2e8f0",
...
# ── 5. Sidebar Controls & Thresholds ──
with st.sidebar:
    st.markdown("### 🧠 Platform Navigation")
    
    nav = st.radio(
        "Navigation",
        ["📊 Executive Overview", "⚠️ Attrition Risk", "🎯 O*NET Skill Gaps", "👤 Employee Dossier", "🤖 AI HR Copilot"],
    ...
    high_risk_thresh = st.slider(
        ...
        format="%d%%",
        help="Probability threshold for critical flight risk classification."
    )
    
    med_risk_thresh = st.slider(
        "🟡 Medium Risk Cutoff",
        min_value=15, max_value=min(60, high_risk_thresh - 5),
        value=min(st.session_state.med_thresh, high_risk_thresh - 5),
        format="%d%%",
        help="Probability threshold for proactive watch list."
    )
...
# ── 6. Load Data & Apply Dynamic Risk Re-Bucketing ──
...
<div class="brand-header">
  <div>
    <div class="brand-title">🧠 Enterprise HR AI <span>// Workforce Intelligence</span></div>
    <div class="brand-subtitle">Real-time XGBoost Attrition Forensics & O*NET Competency Modeling</div>
  </div>
  <div>
    <span class="status-pill">● System Live & Active</span>
  </div>
</div>
""", unsafe_allow_html=True)
...
if nav == "📊 Executive Overview":
    if intel_df is not None:
        ...
        <div class="kpi-container">
          ...
          {make_kpi_card("🚨", f"{high}", f"High Flight Risk", f"{high/total:.1%} of workforce", "#ef4444")}
          {make_kpi_card("⚠️", f"{med}", f"Medium Watchlist", f"{med/total:.1%} of workforce", "#f59e0b")}
          {make_kpi_card("📈", f"{avg_prob:.1%}", "Avg Flight Prob", "Model confidence", "#38bdf8")}
        ...
        if dept_scores_df is not None and not dept_scores_df.empty:
            st.markdown('<div class="section-bar">📈 <span>Department Engagement & Performance Benchmark (5,000-Employee Data)</span></div>', unsafe_allow_html=True)
        ...
        st.error("Employee intelligence data not found. Please verify processed files exist.")


# ════════════════════════════════════════════════
# VIEW 2: ATTRITION RISK
# ════════════════════════════════════════════════
elif nav == "⚠️ Attrition Risk":
    st.markdown('<div class="section-bar">⚠️ <span>Predictive Attrition Risk Analytics</span></div>', unsafe_allow_html=True)
    ...
    if intel_df is not None:
        ...
        if risk_filter != "All Severity Levels":
...
# ════════════════════════════════════════════════
# VIEW 3: O*NET SKILL GAPS
# ════════════════════════════════════════════════
elif nav == "🎯 O*NET Skill Gaps":
    ...
    if org_gap_df is not None:
        ...
        with col_sf:
            sev_filter = st.selectbox("Severity Classification", ["All Deficits", "HIGH", "MEDIUM", "LOW"])
        with col_sn:
            top_n = st.slider("Display Top N Skills", 10, 50, 20)
        ...
        col_h, col_dl = st.columns([4, 1])
        with col_h:
            st.markdown('<div class="section-bar">📊 <span>Competency Detail Register</span></div>', unsafe_allow_html=True)
        with col_dl:
...
# ════════════════════════════════════════════════
# VIEW 4: EMPLOYEE DOSSIER & RETENTION LAB
# ════════════════════════════════════════════════
elif nav == "👤 Employee Dossier":
    ...
    if intel_df is not None:
        ...
        with col_s1:
            search_query = st.text_input("Lookup Employee (Enter ID: 1–500, or Name)", placeholder="e.g. 1, 42, or Steven Barnett...")
        with col_s2:
        ...
        if found_emp is not None:
            ...
            st.caption("Model the projected flight-risk reduction when implementing tailored retention interventions.")
...
# ════════════════════════════════════════════════
# VIEW 5: AI HR COPILOT (LLM CHATBOT)
# ════════════════════════════════════════════════
elif nav == "🤖 AI HR Copilot":
    ...
    if "messages" not in st.session_state:
        st.session_state.messages = [{
            "role": "assistant",
            "content": "👋 **Hello! I am your Enterprise HR AI Copilot.** I have live access to our **XGBoost Attrition Model**, **O*NET Competency Matrix**, and **Employee Intelligence Database**.\n\n"
                       "💡 **You can ask me:**\n"
