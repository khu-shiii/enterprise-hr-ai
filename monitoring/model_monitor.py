...
def compute_metrics(y_true, y_pred_prob, threshold: float = 0.5) -> dict:
    ...
    metrics = {
        ...
        "roc_auc": round(roc_auc_score(y_true, y_pred_prob), 4) if len(set(y_true)) > 1 else None,
...
def run_model_monitor(pred_path: str, actuals_path: str) -> dict:
    ...
    metrics["retrain_recommended"] = retrain_needed
    if retrain_needed:
    ...
    report_path = REPORT_DIR / f"model_monitor_{datetime.utcnow().strftime('%Y%m%d_%H%M')}.json"
    with open(report_path, "w") as f:
        json.dump({"run_at": datetime.utcnow().isoformat(), "metrics": metrics}, f, indent=2)
